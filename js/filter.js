...
const DEBOUNCE_INTERVAL = 500; // минимальный интервал между применениями фильтра
let lastTimeout; // переменная для присваивания параметра функции задержки

window.mapFilters.addEventListener(`change`, function () { // фильтрация меток при изменении опций селектора типа жилья

  if (lastTimeout) {
    window.clearTimeout(lastTimeout); // если время от предыдущего изменения фильтра до текущего меньше DEBOUNCE_INTERVAL, то применение предыдущих изменений фильта отменяются
  }
  lastTimeout = window.setTimeout(function () { // функция задержки срабатывания фильтра
    ...
    window.beforeFilterPinsImages = window.map.querySelectorAll(`.map__pin:not(.map__pin--main) img`); // извлекаем изображения в существующих метках существующие до применения текущего фильтра
    window.beforeFilterPins = window.map.querySelectorAll(`.map__pin:not(.map__pin--main)`); // извлекаем существующие метки до применения текущего фильтра
